...
export default function IndividualResultPage() {
  ...
  useEffect(() => {
    ...
    fetch(
      `/api/results/${encodeURIComponent(String(id))}?lang=${language}`,
      {
        cache: "no-store",
      }
    )
      ...
      .then((data) => {
        if (data.success && data.result) {
          ...
          setDatabaseResult({
            ...
            breakdown: Array.isArray(match.breakdown)
              ? match.breakdown
              : [],
            winnerName: match.winnerName ?? null,
            winnerPhone: match.winnerPhone ?? null,
          });
        }
      })
      .catch((error) => {
              ...
              <div className="mt-5 overflow-hidden rounded-2xl border border-black/10 bg-white shadow-sm sm:mt-6">
                <div className="border-b border-black/10 p-4 sm:p-5">
                  ...
                  <p className="mt-1 text-xs leading-5 text-black/45">
                    Bidder identities are private. Counts show exactly how
                ...
                <div className="overflow-x-auto">
                  <table className="w-full min-w-[460px] text-left text-xs sm:min-w-[500px] sm:text-sm">
                    ...
                    <tbody>
                      {databaseResult.breakdown.map((bid) => (
                        <tr
          ...
          <div className="mx-auto max-w-7xl px-5 py-12 sm:px-6 sm:py-14 lg:px-10 lg:py-16">
            ...
            <div className="mt-8 grid gap-3 sm:mt-10 sm:gap-4 md:grid-cols-3">
              <RuleCard
                ...
                description={t("publishedWinnerDescription")}
              />

              <RuleCard
                icon={<CheckCircle2 size={18} />}
                title={t("finalResult")}
                description={t("finalResultDescription")}
              />

              <RuleCard
                icon={<Users size={18} />}
                title={t("participationRecord")}
                description={t("participationRecordDescription")}
              />
            </div>
          </div>
        </section>
